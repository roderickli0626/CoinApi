...
namespace CoinApi.Controllers
{
    ...
    public class MasterController : BaseController
    {
        ...
        #region PayPal
        [HttpPost("GetAllPayPal")]
        public IActionResult GetAllPayPal(bool isAll = false)
        {
            ...
            try
            {
                ...
                var sortColumnDirection = isAll ? "" : Request.Form["order[0][dir]"].FirstOrDefault();
                var searchValue = isAll ? "" : Request.Form["search[value]"].FirstOrDefault();
                var obj = _masterService.GetAllPayPal(searchValue, sortColumn, sortColumnDirection, start, length, isAll).Result;

                var output = JsonConvert.DeserializeObject<DataTableResponseVM>(obj.Data);
                data = JsonConvert.DeserializeObject<List<tblPayPalDto>>(output.Response);
                var modifiedData = data.Select(d =>
                    new
                    {
                        d.Id,
        ...
        [HttpPost("AddUpdatePayPal")]
        public async Task<IActionResult> AddUpdatePayPal(tblPayPalDto model)
        {
            return Ok(await _masterService.AddPayPal(model));


        }
        [HttpGet("GetPayPalInfoById")]
        public async Task<IActionResult> GetPayPalInfoById(int id)
        {
            return Ok(await _masterService.GetPayPalInfoById(id));
        }

        [HttpPost("DeletePayPal")]
        public async Task<IActionResult> DeletePayPal(int id)
        {
            return Ok(await _masterService.DeletePayPal(id));
        }
        #endregion
    }
}
